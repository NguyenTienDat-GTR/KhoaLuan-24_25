...
const useAppointmentRequestStore = create((set) => ({
    ...
    getRequestAccepted: async (token, { filters }) => {
        set({ loading: true });
        try {
            const params = {};
            ...
            const response = await axios.get("/appointment-request/accepted", {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
                params: params,
            ...
            if (response.status === 200) {
                set({ requestAccepted: response.data.requests, loading: false });
            }
        } catch (error) {
            set({ error: error?.response?.data.message, loading: false });
        }
